...
func (d BuffBatchReader) ReadDeals(filePath string, batchSize int) (results chan []model.Deal) {
	// Buffered channel to store the results.
	results = make(chan []model.Deal, batchSize)
	...
	go func() {
		...
		for {
			...
			deal := model.Deal{
				CustomerID:        record[0],
				OrderID:           record[1],
				CustomerFirstName: record[2],
				CustomerLastName:  record[3],
				OrderDate:         record[4],
				OrderValue:        record[5],
			}

			batch = append(batch, deal)
